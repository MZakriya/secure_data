    ...
    if not st.session_state['logged_in']:
        ...
        if st.button("Submit Login"):
            ...
            if is_locked_out(login_email):
                lockout_time_remaining = int(
                    (
                        st.session_state['failed_attempts'][login_email]['lockout_end']
                        - time.time()
                    )
                )
                st.error(
                    f"\U0001F512 Too many failed attempts. Try again in {lockout_time_remaining} seconds."
                )
                ...
                if st.session_state['failed_attempts'][login_email]['count'] >= FAILED_ATTEMPTS_LIMIT:
                    st.session_state['failed_attempts'][login_email][
                        'lockout_end'
                    ] = time.time() + LOCKOUT_TIME
                    st.warning(
                        (
                            f"\U0001F512 Too many failed attempts. User {login_email} "
                            f"is locked out for {LOCKOUT_TIME} seconds."
                        )
                    )
                else:
                    st.warning(
                        f"❌ Incorrect credentials. Attempts: "
                        f"{st.session_state['failed_attempts'][login_email]['count']}/"
                        f"{FAILED_ATTEMPTS_LIMIT}"
                    )
    else:
